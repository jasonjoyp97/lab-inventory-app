...
# --- PAGE CONFIG ---
st.set_page_config(page_title="SCTIMST ECD Lab", layout="wide")
...
# ==============================================================================
# VIEW 2: DEDICATED LAB PUBLICATIONS PORTAL
# ==============================================================================
else:
    st.title("📚 Laboratory Research & Publications")
    st.caption("Peer-reviewed journals, conference proceedings, and patents from the laboratory.")
    st.divider()
    ...
    pub_results = get_data(query, tuple(params))

    # --- PUBLICATION CARDS RENDERER ---
    if not pub_results.empty:
        st.write(f"Showing **{len(pub_results)}** published work(s):")
        for idx, row in pub_results.iterrows():
            with st.container(border=True):
    ...
    # ==========================================================================
    # ADMIN PANEL: INGESTION & HARVESTING ENGINE
    # ==========================================================================
    if st.session_state.current_role == "admin":
        st.divider()
